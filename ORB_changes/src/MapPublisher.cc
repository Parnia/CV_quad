...
//    publisher = nh.advertise<visualization_msgs::Marker>("ORB_SLAM/Map", 10);
    parnia_pub = nh_par.advertise<geometry_msgs::Point>("ORB_SLAM/Par", 10);
...
//    msgs_p4.z=p4w.at<float>(2);
    ...
    parnia_pub.publish(msgs_o);
//    mCurrentCamera.points.push_back(msgs_o);

    //qDebug() << msgs_o.x;
//    int x_=100*msgs_o.x;
//    int height_=-100*msgs_o.y;
//    int y_=100*msgs_o.z;
//    transmiter->Fill_Data(3,(int)x_,(int)y_,(int)height_);
    //transmiter->Fill_Data(2,100,200);
    //qDebug() << transmiter->data[0];
